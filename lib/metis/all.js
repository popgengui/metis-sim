 ...
 * @param callback
 */
let sim_do_async_cycle = (state, callback) => {
 ...
 * @param {integer} operator
 */
let sim_do_n_cycles = (n, state, callback) => {
    //XXX Doc to review
    ...
    //XXX The CycleStopOperator should be removed
    if (callback) {
        sim_do_async_cycles(state, callback)
    }
    else {
        return sim_do_unspecified_cycles(individuals, add_operators, previous_cycle)
    }
}
 ...
 * @param {integer} operator
 */
let sim_do_unspecified_cycles = (state) => {
    //XX Doc to review
...
const sim_do_async_cycles = (state, callback) => {
	...
	state.cycle += 1
        return_callback = sim_do_async_cycles.bind(state, callback) 
    }
...
class ops_rep_SexualReproduction extends ops_rep_BaseReproduction {
     ...
     */
    constructor (species, size, annotators=[], mater=ops_rep_RandomMater,
...
class ops_rep_NoGenomeSexualReproduction extends ops_rep_SexualReproduction {
    constructor (species, size, annotators=[], mater=ops_rep_RandomMater) {
        super(species, size, annotators, mater, ops_rep_NoGenomeSexualGenerator)
    }
}
...
class ops_rep_StructuredSexualReproduction extends ops_rep_BaseReproduction {
     ...
     *   reproductor (this) and annotators. The rest will be default 
     */
    constructor (species, pop_size, num_pops, annotators=[], mater=ops_rep_RandomMater,
        generator=ops_rep_SexualGenerator) {
...
class ops_rep_NoGenomeStructuredSexualReproduction extends ops_rep_StructuredSexualReproduction {
    constructor (species, pop_size, num_pops, annotators=[], mater=ops_rep_RandomMater) {
        super(species, pop_size, num_pops, annotators, mater, ops_rep_NoGenomeSexualGenerator)
    }
}
