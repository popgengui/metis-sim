...
/* A clarifier for the cum allele stats object's arrays */
const NO_CYCLE_RECORDED=-1
...
const gn_make_cum_marker_stats_holder = ( num_markers ) => {
	...
	let cycle_at_locus_fixed = []
	let cycle_at_allele_fixed = []
	let cycle_at_allele_lost = []

	let total_loci_fixed=0
	let total_allele_fixed=0
	let total_allele_lost=0

	for (let i=0; i < num_markers; i++ ) {
		cycle_at_locus_fixed.push( NO_CYCLE_RECORDED )
		cycle_at_allele_fixed.push( NO_CYCLE_RECORDED )
		cycle_at_allele_lost.push( NO_CYCLE_RECORDED )
	}//end for each marker

	cum_marker_stats.cycle_at_locus_fixed = cycle_at_locus_fixed
	cum_marker_stats.cycle_at_allele_fixed = cycle_at_allele_fixed
	cum_marker_stats.cycle_at_allele_lost = cycle_at_allele_lost

	cum_marker_stats.total_loci_fixed = total_loci_fixed
	cum_marker_stats.total_allele_fixed = total_allele_fixed
	cum_marker_stats.total_allele_lost = total_allele_lost

	return cum_marker_stats
}//end gn_make_cum_marker_stats_holder
...
}//end class ops_stats_demo_SexStatistics
...
class ops_stats_FreqAl extends ops_GenomeCountStatistics {
    constructor({ name='FreqAl', allele=1, doMean=false }) {
    ...
    /*20200407.  This version of compute_counts was copied over
     * from the develop version, to include the mean calculation.*/
...
class ops_stats_hz_ExpHeDeme extends ops_DemeStatistics {
    constructor( doMean=false ) {
        super('ExpHe', () => new ops_stats_hz_ExpHe( {doMean:doMean} ))
    }
}

/*20200406  Ted adds the operator to get totals for the
 * cum_marker_stats object at each cycle
 */

class ops_CumAlleleTotals extends ops_GenomeCountStatistics {
    constructor( { name='CumAlleleStats', 
		allele=1, total_markers=0 } ) 
    {
        super(name)
	this.allele=allele
	this.o_cum_stats=gn_make_cum_marker_stats_holder( total_markers ) 
    }//end constructor
    
    compute_counts(counts) 
    {
        let this_cycle_number= this._state.cycle
        let freq_allele = {}
        ...
        for (let marker in counts) 
	{
		...
		/*We don't need to compute the allele counts if this marker is already fixed*/
		let b_locus_fixed_in_previous_cycle = 
			 (this.o_cum_stats.cycle_at_locus_fixed[ feature_count ] !== NO_CYCLE_RECORDED)

		if( !b_locus_fixed_in_previous_cycle )
		{
			...
			/*Count will be zero if allele1 was lost, and will 
			* equal all_counts if allele1 was fixed. Note that 
			* in both cases the locus is fixed, and so will
			* will be so recorded.*/
			let thiscount=ft_counts[this.allele] || 0 

			if( thiscount === 0 )
			{

				if( this.o_cum_stats.cycle_at_locus_fixed[ feature_count ] === NO_CYCLE_RECORDED )
				{
					this.o_cum_stats.cycle_at_locus_fixed[ feature_count ] = this_cycle_number
					this.o_cum_stats.total_loci_fixed++
				}//end if new fixed loci for exphe

				if( this.o_cum_stats.cycle_at_allele_lost[ feature_count ] === NO_CYCLE_RECORDED )
				{
					this.o_cum_stats.cycle_at_allele_lost[ feature_count ] = this_cycle_number
					this.o_cum_stats.total_allele_lost++
				}//end if new fixed allele for freq allele

			}else if( thiscount === all_counts )
			{
				...
				if( this.o_cum_stats.cycle_at_allele_fixed[ feature_count ] === NO_CYCLE_RECORDED )
				{
					this.o_cum_stats.cycle_at_allele_fixed[ feature_count ] = this_cycle_number
					this.o_cum_stats.total_allele_fixed++
				}//end if this is first gen at which allele is fixed
			}//end if freq is zero else if freq is one
		     }//end if this locus (feature) was not already fixed, then compute current count for allele1
		  }//end for each ft_count
	}//end for each marker
	return  this.o_cum_stats
    }//end compute_counts

}//end class ops_stats_cum_allele
...
/*20200406 Ted adds new operator*/
expt.ops_CumAlleleTotals = ops_CumAlleleTotals
