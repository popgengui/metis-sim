...
class ops_stats_hz_ExpHe extends ops_GenomeCountStatistics {
    ...
    compute_counts(counts) {
        ...
        let sum_exp_he=0
        let marker_count=0
	for ( let this_marker in exp_he ) {
            sum_exp_he += exp_he[ this_marker ]
            marker_counts += 1
        }//end for each exp he
	exp_he[ 'mean' ] = sum_exp_he / marker_counts

        return exp_he
    }
}

class ops_stats_mean_hz_ExpHe extends ops_GenomeCountStatistics{
    constructor( name = 'MeanExpHe' ) {
        super( name )
    }
	...
	let exp_he_count=0
        for (let this_marker in exp_he ) {
            exp_he_count += 1
            mean_exp_he += exp_he[ this_marker ]
        }
        mean_exp_he /= exp_he_count

        return mean_exp_he
    }//end compute_counts
}//end class ops_stats_mean_hz_ExpHe
